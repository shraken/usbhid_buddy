...
BUDDY_DAC_DYNAMIC_CHECK
    ...
    :FOR  ${channel}  IN RANGE  0  ${BUDDY_NUMBER_CHANNELS} + 1
    \    ${result}=  Buddy Dynamic ADC Check  ${channel}  ${BUDDY_DYN_FREQ}  ${BUDDY_DYN_WAVE_FREQ}
    #\    Log  ${result}  console=yes
    \    ${min} =      Evaluate    ${BUDDY_DYN_WAVE_FREQ} - ${BUDDY_THRESH_PLUSMINUS}
    \    ${max} =      Evaluate    ${BUDDY_DYN_WAVE_FREQ} + ${BUDDY_THRESH_PLUSMINUS}
