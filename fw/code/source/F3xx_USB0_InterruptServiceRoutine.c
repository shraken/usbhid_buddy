...
void Handle_In2 (void);                // Handle in packet on EP 1
void Handle_Out2 (void);               // Handle out packet on EP 1
...
void Usb_ISR(void) __interrupt (INTERRUPT_USB0)
{
   unsigned char bCommon, bIn, bOut;

   //printf("Usb_ISR invoked\r\n");
...
void Handle_Control (void)
{
   ...
   if (EP_STATUS[0] == EP_IDLE)        // If Endpoint 0 is in idle mode
   {
      ...
      {                                // ready from host although if EP0
         ...
         SETUP.wValue.i = SETUP.wValue.c[LSB] + 256*SETUP.wValue.c[MSB];
         SETUP.wIndex.i = SETUP.wIndex.c[LSB] + 256*SETUP.wIndex.c[MSB];
         SETUP.wLength.i = SETUP.wLength.c[LSB] + 256*SETUP.wLength.c[MSB];
